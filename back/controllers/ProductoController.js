...
const eliminar_inventario_producto_admin = async function (req, res) {
  if (req.user) {
    if (req.user.role == "admin") {
      ...
      let reg = await Inventario.findOneAndDelete({ _id: id });
      //Obtener el producto
      let prod = await Product.findById({ _id: reg.producto });
      //Actualizar el stock
      let nuevo_stock = parseInt(prod.stock) - parseInt(reg.cantidad);
      //Actualizar en la tabla productos
      let producto = await Product.findByIdAndUpdate({ _id: reg.producto }, {
        stock: nuevo_stock
      });
...
const listar_productos_recomendados_publico = async function (req, res) {
      ...
      let reg = await Product.find({ categoria: categoria }).sort({ createdAt: -1, }).limit(10);
      res.status(200).send({ data: reg });
}
